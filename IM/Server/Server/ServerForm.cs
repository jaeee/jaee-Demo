...
namespace Server
{
    ...
    public partial class ServerForm : Form
    {
        ...
        int tcpPort;
        // 定义变量
        //这里使用类UDPclient实现收发信息
        //IPAddress 提供了对IP地址的转换、处理等功能
        //IPEndPoint 类包含应用程序连接到主机上的服务所需的主机和端口信息，
        //通过组合服务的主机IP地址和端口号，IPEndPoint类形成到服务的连接点。
        
        private UdpClient sendUdpClient;//发消息
        private UdpClient receiveUdpClient;//收消息
        private IPEndPoint serverIPEndPoint;//套接字创建
        private TcpListener tcpListener;
        private IPAddress serverIp;
        private NetworkStream networkStream;//接收套接字 类比accept
        private BinaryWriter binaryWriter;//写入
        ...
        public ServerForm()
        {
            ...
            txbServerIP.Text = serverIPs[6].ToString();
        ...
        {
            // 创建服务端接收套接字
            serverIp = IPAddress.Parse(txbServerIP.Text);//将输入框内的ip从string转成IP
            //创建服务端套接字
